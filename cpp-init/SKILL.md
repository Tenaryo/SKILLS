...
### Core Files
- `CMakeLists.txt` — Root CMake config (C++23, INTERFACE library, strict warnings, sanitizer option)
...
### Test Infrastructure
- `tests/CMakeLists.txt` — Auto-discovers `test_*.cpp` files, one executable per file
- `tests/test_main.cpp` — Minimal test (new projects only)
...
- **No test framework** — use raw `assert()` for zero-dependency testing
- **One test executable per `test_*.cpp` file** in `tests/`
